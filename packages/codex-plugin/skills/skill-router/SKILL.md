...
3. Read the canonical registry evidence: top-level `source`, `registry.mode`, and for each result `match_score`, `skillradar_score`, `security`, `source`, and `reason`.
...
For explicit routing/recommendation requests, include enough evidence to prove the bundled Registry was used:
